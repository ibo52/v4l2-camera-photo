 ...
 * which comes as builtin on linux kernel.
...
#include "Camera.h"
							//for terminals not support color ESC values
#define CAM_COLORFUL_PRINT 1// we can remove tah color values by setting this to 0

#if(CAM_COLORFUL_PRINT==1)
...
int get_frameSize(Camera* self, int index){
...
int get_format(Camera* self, int fmt_description_index){	//if 1: get all formats; else get first format
...
static int camera__get_fps(Camera* self){	//if 1: get parameters to 'struct param' (fps denominator numerator etc.)
...
void camera__control__enumerate_menu(Camera* self){
...
int camera__control__get_ctrl(Camera* self){
	/*
...
static void set_format(Camera* self, int width,int height){
...
static int init_camera(Camera* self){
...
static int init_mmap(Camera* self)
{
    ...
    self->buffer=calloc(self->specs.req.count, sizeof(cameraBuffer));
...
static int ready_to_capture(Camera* self){
/*
...
static int dequeue_buff(Camera* self){
...
static void close_device(Camera* self){
        if (-1 == close(self->fd)){
...
int camera__deactivate(Camera* self){
...
cameraBuffer camera__capture(Camera* self, int buffer_type){

	cameraBuffer DataHolder={0};//clear(sort of null) buffer
...
static const char* getCurrentTime(void){
	...
	strftime(strTime, 32 ,"-%d_%b_%Y_%H:%M:%S",local_curr);

	return strTime;
	
}

const char* camera__imsave(Camera* self, const char* name){

	//char* filename=calloc(129,sizeof(char));
	static char filename[129]={0};
	
	//strcat(filename,"images/");
	strcat(filename, name);
	strcat(filename, getCurrentTime());
...
Camera* camera__new(const char* path){

	Camera* obj=(Camera*)calloc(1, sizeof(Camera));
...
void camera__destroy(Camera** self){
